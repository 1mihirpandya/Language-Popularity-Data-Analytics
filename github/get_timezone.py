...
def timezone(sc, sqlContext, langs):
    ...
    no_null_langs = no_null_langs.drop('repo').drop('author_date')
   
    to_rdd = no_null_langs.rdd
    '''
        for each weekday - the date() returns 0-7 (0 being monday)
    '''
    rdd = to_rdd.map(list)
    fix_date = rdd.map(lambda x: get_timezone(x))
